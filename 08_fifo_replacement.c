...
// pretty much the same as lru replacement with one statement commented out
...
void solve()
{
	...
	for(int t = 0; t < reference_string_size; ++t)
	{
		...
		{
			nf_hits ++;
			// times_of_entry[pos] = t;
		}
		else if (nf_occupied != n)
		{
			arr[nf_occupied] = key;
			times_of_entry[nf_occupied] = t;
			nf_occupied ++;

		}
		else
		{
			int loc = location_of_min(times_of_entry, n);
			arr[loc] = key;
			times_of_entry[loc] = t;
		}
		ps(key); 
