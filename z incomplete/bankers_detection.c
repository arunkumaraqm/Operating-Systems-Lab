...
void read_data(
	...
	int* requesting_process,
	int request_made[nf_resources]
	)
{
	...
	for (int rs = 0; rs < nf_resources; ++rs)
	{
		scanf("%d", request_made[rs]);
		// request is invalid if it is more than the current need of the process
		assert(request_made[rs] <= need_mat[requesting_process][rs]);
	}
}
...
int main(int argc, char const *argv[])
{
	...
	int request_made[nf_resources], requesting_process;
	int safe_sequence[nf_processes]; 
